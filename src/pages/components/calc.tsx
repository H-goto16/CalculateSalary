...
export const Calc = () => {
  ...
  useEffect(() => {
    if (changeTrunc === true) {
      ...
      setSaralyData(
        Math.trunc(culcMoney(hourlyWageData, hourData, minutesData))
      );
  ...
  return (
    <>
      <input
        ...
        className="inputData"
        placeholder="時給"
        onChange={(e) => setHourlyWageData(e.target.value as unknown as number)}
      />
      <br />
      <input
        type="number"
        className="inputData"
        placeholder="時間"
        onChange={(e) => setHourData(e.target.value as unknown as number)}
      />
      <br />
      <input
        type="number"
        className="inputData"
        placeholder="分"
        onChange={(e) => setMinutesData(e.target.value as unknown as number)}
      />
      <br />
      <input
        type="number"
        className="inputData"
        placeholder="日"
        onChange={(e) => setDayData(e.target.value as unknown as number)}
      />
      <br />
      <input
        type="number"
        className="inputData"
        placeholder="予算"
        onChange={(e) => setBudget(e.target.value as unknown as number)}
      />
      <br />
      <button
        type="button"
        className="button"
        onClick={() => setChangeTrunc((current) => !current)}
      >
        {buttonTitle}
      </button>
      ...
      <h1 className="salary">{budgetTime}</h1>
    </>
  );
};
